...
RSpec.feature "SubjectLayouts", type: :feature, js: true do
  ...
  describe "subjects page" do
    ...
    it "has areas" do
      expect(page).to have_css("cap-areas")
    end
  end
  ...
  shared_examples "area" do |area, tabs|
    it "has #{area[:label]} area component" do
      within("cap-areas") do
        ...
        expect(page).to have_css("cap-area[label='#{area[:label]}']")
        within("cap-area[label='#{area[:label]}']") do
    ...
    it "#{area[:label]} area has tabs" do
      within("cap-area[label='#{area[:label]}'] div.area-pane") do
        expect(page).to have_css("cap-tabs") 
          ...
          within("div.tabs-pane") do
            ...
            tabs.each_with_index do |tab,idx|
              ...
              within("div.tab-content") do #content has our component for tab.label
                expect(page).to have_css("cap-tab[label='#{tab[:label]}']")
                within("cap-tab[label='#{tab[:label]}']") do
    ...
    it "#{area[:label]} area is collapsable" do
      find("cap-area[label='#{area[:label]}'] div.area-pane")
  ...
  describe "area sides" do
    it "displays right and left" do
      ["left", "right"].each do |pos|
        expect(page).to have_css("cap-areas div.areas-#{pos}")
        expect(page).to have_no_css("cap-areas div.areas-#{pos}.ng-hide")
        within("cap-areas div.areas-#{pos}") do
          expect(page).to have_css("cap-area:nth-child(1)")
        end
      end
    end
    ...
    it "displays right fullsize when left hidden" do
      ...
      ["Subjects","Details"].each do |area|
        within("cap-area[label='#{area}']") do
          find("div.area-pane input[name='collapse-area']").click  #collapse
        end
      end
      expect(page).to have_no_css("cap-areas div.areas-left")
      expect(page).to have_css("cap-areas div.areas-right.col-sm-12")
    end
    ...
    it "displays left fullsize when right hidden" do
      ...
      expect(page).to have_no_css("cap-areas div.areas-right")
      expect(page).to have_css("cap-areas div.areas-left.col-sm-12")
    end

    it "expands upper left when lower left hidden" do
      expect(page).to have_css("cap-area[label='Subjects'] div.area-pane") 
      expect(page).to have_css("cap-area[label='Details'] div.area-pane") 
      expect(page).to have_no_css("cap-area[label='Subjects'] div.area-pane.expanded") 
      expect(page).to have_no_css("cap-area[label='Details'] div.area-pane.expanded") 

      find("cap-area[label='Details'] input[name='collapse-area']").click  #collapse
      expect(page).to have_no_css("cap-area[label='Details'] div.area-pane") 
      expect(page).to have_css("cap-area[label='Subjects'] div.area-pane.expanded") 
    end
    ...
    it "expands lower left when upper left hidden" do
      ...
      find("cap-area[label='Subjects'] input[name='collapse-area']").click  #collapse
      expect(page).to have_no_css("cap-area[label='Subjects'] div.area-pane") 
      expect(page).to have_css("cap-area[label='Details'] div.area-pane.expanded") 
    end
  end
end